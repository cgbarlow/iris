...
| **Markdown (Smart Markdown)** *(v6.14.0)* | Markdown source with inline `/`-triggered references to live entity fields. Token format `{{<entity-type>:<id>:<field-spec>}}` resolves at read time — reference any element's `name`, `description`, or custom `attr:<key>` (and `name`/`description` of any package/diagram/set/collection) | Two-step slash picker (entity then field); unresolvable tokens render as `~~{{...}}~~` so data loss is visible; resolved content lands in `data.content` so existing markdown/docx/pdf exports work unchanged |
...
- **AI "Create Diagram" across five notations** — the guided creation flow (ADR-094 / ADR-132) supports DoView (outcomes_map, overview), Simple (component, roadmap, free-form, class), UML (sequence, class), ArchiMate (process), and C4 (deployment). Pick a notation and (for non-DoView) a diagram type; the AI walks through setup questions, structure confirmation, content confirmation, then generates end-to-end
...
- SparxEA native XMI 2.1 import (.xml): Enterprise Architect's "Export Package to XMI 2.1 / Native XML" output — packages, elements, connectors, tagged values, and diagram geometry, reusing the same pipeline as the .qea importer (ADR-219). The import page content-sniffs `.xml` uploads to route Sparx native XMI vs ArchiMate Open Exchange automatically
