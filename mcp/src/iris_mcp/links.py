...
def _orient_wrapper(kind: str, entity_id: str) -> str:
    ...
    return (
        ...
        f"\n"
        f"  3. Offer the menu options below to the user via "
        f"AskUserQuestion when your client exposes a structured "
        f"user-question tool (Claude Code, Claude Desktop, claude.ai, "
        f"Cursor — they all do). Fire ONE AskUserQuestion call carrying "
        f"all four options, IN ORDER, with each option's full menu "
        f"sentence in the option `description` field — that text is "
        f"LITERAL and must be copied CHARACTER-BY-CHARACTER from the "
        f"orient sheet's MENU section. Do NOT summarise. Do NOT shorten. "
        f"Do NOT drop parenthetical examples like \"(e.g. J06 — "
        f"Mathematization of Outcomes Theory)\". Long options are "
        f"intentional. Derive a short (3-5 word) `label` for each "
        f"option from its leading concept so the chip stays legible.\n"
        f"     When your client does NOT expose a user-question tool, "
        f"fall back to a numbered prose list (1. 2. 3. 4.) with each "
        f"option's full sentence on its own line — same verbatim rules "
        f"apply. Do NOT render the menu as prose bullets when "
        f"AskUserQuestion is available — the user expects clickable "
        f"chips, not a paragraph.\n"
        f"\n"
