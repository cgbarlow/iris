"""Integration tests for diagram CRUD API routes."""
...
class TestCreateDiagram:
    ...
    async def test_create_returns_201(self, client: httpx.AsyncClient) -> None:
        ...
        resp = await client.post(
            "/api/diagrams",
            json={"diagram_type": "simple-view", "name": "My Diagram", "data": {}},
            headers=headers,
        )
        assert resp.status_code == 201
    ...
    async def test_create_requires_auth(self, client: httpx.AsyncClient) -> None:
        resp = await client.post(
            "/api/diagrams",
            json={"diagram_type": "simple-view", "name": "test", "data": {}},
        )
        assert resp.status_code == 401


class TestGetDiagram:
    """Verify diagram retrieval."""

    async def test_get_diagram(self, client: httpx.AsyncClient) -> None:
        headers = await _auth_headers(client)
        created = await _create_diagram(client, headers)
        resp = await client.get(
            f"/api/diagrams/{created['id']}", headers=headers,
        )
        assert resp.status_code == 200
        assert resp.json()["name"] == "Test Diagram"

    async def test_get_nonexistent_returns_404(
        self, client: httpx.AsyncClient,
    ) -> None:
        headers = await _auth_headers(client)
        resp = await client.get("/api/diagrams/nonexistent", headers=headers)
        assert resp.status_code == 404


class TestListDiagrams:
    """Verify diagram listing."""
    ...
    async def test_filter_by_type(self, client: httpx.AsyncClient) -> None:
        headers = await _auth_headers(client)
        await _create_diagram(client, headers, diagram_type="simple-view")
        await _create_diagram(client, headers, diagram_type="uml-class")
        resp = await client.get(
            "/api/diagrams?diagram_type=uml-class", headers=headers,
        )
        assert resp.json()["total"] == 1


class TestUpdateDiagram:
    """Verify diagram update."""
...
class TestDiagramVersions:
    ...
    async def test_get_versions(self, client: httpx.AsyncClient) -> None:
        ...
        created = await _create_diagram(client, headers)
        resp = await client.get(
            f"/api/diagrams/{created['id']}/versions", headers=headers,
        )
        assert resp.status_code == 200
...
class TestDiagramUsernameResolution:
    """Verify GUID-to-username resolution in diagram responses (ADR-031)."""

    async def test_get_diagram_returns_created_by_username(
        self, client: httpx.AsyncClient
    ) -> None:
        headers = await _auth_headers(client)
        created = await _create_diagram(client, headers)
    ...
    async def test_diagram_versions_return_created_by_username(
        self, client: httpx.AsyncClient
    ) -> None:
        headers = await _auth_headers(client)
        created = await _create_diagram(client, headers)
