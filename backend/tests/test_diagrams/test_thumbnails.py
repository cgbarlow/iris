...
_DIAGRAMS_INSERT_DELETED = (
    "INSERT INTO diagrams "
    "(id, diagram_type, current_version, "
...
def _node_data(label: str = "T") -> str:
    """Return JSON diagram data with one node."""
    return json.dumps({
        "nodes": [
...
class TestThumbnailEndpoint:
    ...
    async def test_thumbnail_returns_200_after_create(
        ...
        thumb = await client.get(
            f"/api/diagrams/{diagram_id}/thumbnail",
        )
    ...
    async def test_thumbnail_nonexistent_diagram_returns_404(
        self, client: httpx.AsyncClient,
    ) -> None:
        resp = await client.get(
            "/api/diagrams/nonexistent-id/thumbnail",
        )
...
class TestRegenerateAllThumbnails:
    ...
    async def test_creates_thumbnails_for_diagrams_without_entries(
        ...
        diagram_id = "test-diagram-no-thumb"
        now = datetime.now(tz=UTC).isoformat()

        await db.execute(
            _DIAGRAMS_INSERT, (diagram_id, now, user_id, now),
        )
        await db.execute(
            _VERSIONS_INSERT,
            (diagram_id, "Test", _node_data(), now, user_id),
        )
        await db.commit()

        # Remove any startup-generated thumbnail
        await db.execute(
            "DELETE FROM diagram_thumbnails WHERE diagram_id = ?",
            (diagram_id,),
        )
        await db.commit()

        thumb = await get_thumbnail(db, diagram_id)
    ...
    async def test_updates_stale_svg_thumbnails_to_png(
        ...
        user_id = await _insert_test_user(db)
        diagram_id = "test-diagram-svg-thumb"
        now = datetime.now(tz=UTC).isoformat()
        ...
        await db.execute(
            _VERSIONS_INSERT,
            (diagram_id, "SVG Test", _node_data("S"), now, user_id),
        )
        # Overwrite with stale SVG bytes
        await db.execute(
            _THUMBS_INSERT,
            (diagram_id, "dark", b"<svg>stale</svg>", now),
        )
        await db.commit()

        thumb_before = await get_thumbnail(db, diagram_id)
        assert thumb_before is not None
        assert thumb_before[:8] != PNG_MAGIC

        await regenerate_all_thumbnails(db)

        thumb_after = await get_thumbnail(db, diagram_id)
        assert thumb_after is not None
        assert thumb_after[:8] == PNG_MAGIC

        await db_manager.close()

    async def test_skips_deleted_diagrams(
        self, app_config: AppConfig,
    ) -> None:
        ...
        user_id = await _insert_test_user(db)
        diagram_id = "test-diagram-deleted"
        now = datetime.now(tz=UTC).isoformat()

        await db.execute(
            _DIAGRAMS_INSERT_DELETED,
            (diagram_id, now, user_id, now),
        )
        await db.execute(
            _VERSIONS_INSERT,
            (diagram_id, "Deleted", json.dumps({"nodes": []}), now, user_id),
        )
        await db.commit()
...
class TestStartupThumbnailRegeneration:
    ...
    async def test_startup_creates_thumbnails(
        ...
        user_id = await _insert_test_user(db)
        diagram_id = "startup-test-diagram"
        now = datetime.now(tz=UTC).isoformat()
        ...
        await db.execute(
            _VERSIONS_INSERT,
            (diagram_id, "Startup", _node_data("S"), now, user_id),
        )
        await db.commit()
        await db_manager.close()
...
class TestThemeThumbnails:
    ...
    async def test_thumbnail_endpoint_accepts_theme_param(
        ...
        thumb = await client.get(
            f"/api/diagrams/{diagram_id}/thumbnail?theme=light",
        )
        assert thumb.status_code == 200
        assert thumb.content[:8] == PNG_MAGIC
    ...
    async def test_thumbnail_endpoint_accepts_high_contrast(
        ...
        thumb = await client.get(
            f"/api/diagrams/{diagram_id}/thumbnail?theme=high-contrast",
        )
        assert thumb.status_code == 200
        assert thumb.content[:8] == PNG_MAGIC
    ...
    async def test_regeneration_creates_all_three_theme_variants(
        ...
        user_id = await _insert_test_user(db)
        diagram_id = "test-diagram-themes"
        now = datetime.now(tz=UTC).isoformat()
        ...
        await db.execute(
            _VERSIONS_INSERT,
            (diagram_id, "Theme", _node_data("T"), now, user_id),
        )
        await db.commit()

        # Clear any existing thumbnails for this diagram
        await db.execute(
            "DELETE FROM diagram_thumbnails WHERE diagram_id = ?",
            (diagram_id,),
        )
        ...
        for theme in VALID_THEMES:
            thumb = await get_thumbnail(db, diagram_id, theme=theme)
    ...
    async def test_default_theme_is_dark(
        ...
        thumb_dark = await client.get(
            f"/api/diagrams/{diagram_id}/thumbnail?theme=dark",
        )
...
class TestAdminThumbnailRegeneration:
    ...
    async def test_regeneration_endpoint_returns_200_and_count(
        ...
        headers = await _auth_headers(client)
        # Create a diagram so there is something to regenerate
        await client.post(
            "/api/diagrams",
            json={
                "diagram_type": "simple-view",
