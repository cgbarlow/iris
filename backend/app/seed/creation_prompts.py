...
WORKFLOW GUIDANCE.
Each tool's description carries its own workflow. For diagram creation, see `create_diagram` (it explains the full discover → fetch creation cascade → guided conversation → confirm destination → save flow). To connect elements, call `list_relationships` (by element_id or set_id) to see what exists, then `create_relationships` (batch, same-set only; source_role / target_role for UML role names); fix one with `update_relationship` or remove it with `delete_relationship`. To show a relationship on a diagram, pass its id as the edge's `data.relationshipId` in an `add_edge` op of `patch_diagram` (or in `update_diagram`). For small edits to an existing diagram — adding, moving or removing a few nodes or edges, or drawing new relationships on a large canvas — use `patch_diagram` instead of resending the whole canvas through `update_diagram`: pass `expected_version` (the `current_version` you read) and its operations are applied atomically as one new version. Its `sync_labels` op refreshes node labels to their elements' current names.

AUTH RECOVERY.
If a tool returns error="auth_required", the user needs to sign in to Iris in their MCP client. Tell them: in claude.ai go to Settings → Connectors → Iris and click "Connect" / "Sign in"; a browser tab opens for sign-in and consent. They will NOT be asked for a client_id or secret — Dynamic Client Registration (RFC 7591) handles that automatically. If no sign-in button appears, try removing and re-adding the connector. Read tools (search, get_*, list_*, package_hierarchy) work without sign-in; only writes (create_*, update_*, patch_*, delete_*) need it. Don't call any auth-related tool yourself — the OAuth handshake is between the MCP client and Iris.
"""
