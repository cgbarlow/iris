...
{:else if pkg}
	...
	<div class="flex items-center justify-between">
		...
		<div class="flex gap-2">
			{#if isInContext}
				<button
					onclick={() => removeAiContextItem(pkg.id)}
			...
			{:else}
				<button
					onclick={() => addAiContextItem({ id: pkg.id, result_type: 'package', name: pkg.name, set_id: pkg.set_id ?? null, set_name: pkg.set_name ?? null })}
					class="flex items-center gap-2 rounded px-4 py-2 text-sm"
